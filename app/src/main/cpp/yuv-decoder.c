...
Java_com_hyuan_smallvideo_utils_NativeTool_YUVtoRBGA(JNIEnv *env, jobject obj,
                                                                       jbyteArray yuv420sp,
    ...
    int h = height;
    sz = w * h;
    __android_log_write(ANDROID_LOG_ERROR, "hyuan", "in");
    jint *rgbData = (jint *) ((*env)->GetPrimitiveArrayCritical(env, rgbOut, 0));
    jbyte *yuv = (jbyte *) (*env)->GetPrimitiveArrayCritical(env, yuv420sp, 0);
    __android_log_write(ANDROID_LOG_ERROR, "hyuan", "in 1");
    for (j = 0; j < h; j++) {
        pixPtr = j * w;
        jDiv2 = j >> 1;
        for (i = 0; i < w; i++) {
            ...
            __android_log_write(ANDROID_LOG_ERROR, "hyuan", "in 2");
...
JNIEXPORT void JNICALL
Java_com_hyuan_smallvideo_utils_NativeTool_YUVtoARBG(JNIEnv *env, jobject obj,
                                                                       jbyteArray yuv420sp,
                                                                       jint width, jint height,
                                                                       jintArray rgbOut) {
    int sz;
...
JNIEXPORT void JNICALL
Java_com_hyuan_smallvideo_utils_NativeTool_adjustBitmap(JNIEnv *jenv, jclass thiz,
                                                                       jobject src) {
